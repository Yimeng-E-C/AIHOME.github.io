...
# add more stable/alternative sources
RSS_SOURCES.extend([
...
def fetch_rss(url, max_items=10):
    ...
    if not found:
        # try Atom <entry>
        found = root.findall('.//{http://www.w3.org/2005/Atom}entry') or root.findall('.//entry')

    for entry in found[:max_items]:
        # try common tags
        ...
        if not link:
            # Atom link is an element with href
